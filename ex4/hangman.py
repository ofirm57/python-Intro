...
# DESCRIPTION: program that play hangman game
#################################################################
...
def hint_length_check(matches):
    ...
    if len(matches) > HINT_LENGTH:
        ...
        for i in range(HINT_LENGTH):
            matches_new.append(matches[(i * n) // HINT_LENGTH])
    return matches_new
...
def run_single_game(words_list, score):
    ...
    while score >= 0:
        ...
        score = ask_for_hint(guess, pattern, score, words_list,
                             wrong_guess_lst)
        if score <= 0:
            msg = LOOSE_MSG.format(game_word)
...
def ask_for_hint(guess, pattern, score, words_list, wrong_guess_lst):
    ...
    if guess[0] == HINT:
        score -= 1
        filt_matches = filter_words_list(words_list, pattern, wrong_guess_lst)
        show_suggestions(filt_matches)
    return score
...
def main():
    ...
    game_num = 1
    answer = True
    while answer and score > 0:
        score = run_single_game(words_list, score)
        answer = play_again(GAME_DATA.format(score, game_num))
        game_num += 1
        if score == 0:
            game_num = 1
            score = POINTS_INITIAL
