...
from talpi_tester.autotest import mp_test,res_code,TestSet,announce_failure
from talpi_tester import autotest as at
from talpi_tester import testrunners
...
if __name__=="__main__":
    ...
    run_all_tests("talpi_tester.talpi_tests",set(argv[1:]),dryrun)


    with open('result.log', 'w') as f:
        f.write(def_out.getvalue())
    ...
    """
    if content and 'problem' not in content and "wrong" not in content and "BEGIN" not in content:
        import os
        os.startfile("talpi_tester\\cache_data\\ssh_audio_key.mp3")
        time.sleep(1)
        os.startfile("talpi_tester\\cache_data\\dns_pulse_terminator.jpg")
    """
