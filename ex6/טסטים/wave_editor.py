...
# NISAION_LEMILON =[{'A': 440, 'B':494,'C': 523,'D': 587,'E':659,'F':698,'G':784}]
MAX_VAL = 32767
MIN_VAL = -32768
CHANGE_FILE = '1'
COMPOSE_SOUND = '2'
...
def change_wav_options(lst):
    ...
    while option_input != QUIET:
        if option_input not in OPTIONS_LST:
            ...
            sound_lst = find_the_right_change_path(sample_rate, sound_lst, option_input)
            print(CHANGE_SOUND)
            print(CHANG_OPTIONS_STR)
            option_input = input()
    save = save_menu(sample_rate, sound_lst)
    return save



def find_the_right_change_path(sample_rate, sound_lst, num):
    output_sound = []
    if num == REVERSE_WAV:
...
def composing_per_chord(sample_for_cycle, num_per_six_ten):
    ...
    :param sample_for_cycle:
    :param num_per_six_ten:
    :return:
    """
    new_wave = []
    for i in range(int((DEF_SAMPLE_RATE * int(num_per_six_ten)) / 16)):
        if sample_for_cycle == 0:
            tmp_sample = 0
        else: tmp_sample = int(MAX_VAL * math.sin(2 * PIE * (i / sample_for_cycle)))
...
def build_compose_list(tup_lst):
    ...
    for sum_per_cyc, num_per_sixt in tup_lst:
        print(sum_per_cyc, num_per_sixt)
...
def check_composing_file(the_chord_lst):
    ...
    :param the_chord_lst: the list of the ??????????????????????????
    :return:If the file have a problem returns an error message
    Otherwise returns the  %%tup_lst%%% ?????????????????????????????????
    """
    tup_lst = []
    ...
    for i in range(0, len(the_chord_lst), 2):
        if i == len(the_chord_lst) - 1:
            break
        tup = find_sample_for_cycle(find_freq_by_chord(the_chord_lst[i])), the_chord_lst[i + 1]
        tup_lst.append(tup)
    return tup_lst
...
def change_wav_menu():
    ...
    while output_lst == -1:
        ...
        fle_name = input()
        output_lst = wh.load_wave(fle_name)
    return change_wav_options(output_lst)
...
def main_menu():
    ...
    while choice != 3:
        if choice != str(CHANGE_FILE) and choice != str(COMPOSE_SOUND) and choice != str(EXIT):
            print(WRONG_CHOICE)
            choice = print_and_choice()
            continue
